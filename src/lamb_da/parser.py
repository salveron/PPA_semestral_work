...
from lamb_da.node.nodes import *
from lamb_da.node.unary_operators import *
from lamb_da.node.binary_operators import *
...
class LambdaParser:
    ...
    def __init__(self, symbolic_functions):
        ...
        self.tokenizer = re.compile(r"([()^.TF]|" +
                                    r"|".join([op.re_char for op in unary_operators]) +
                                    r"|".join([op.re_char for op in binary_operators]) +
                                    r")|\s+").split
        ...
        self.literal_re_pattern = re.compile(r"(\d+|[TF]|" +
                                             r"|".join([func for func in self.symbolic_functions.keys()]) +
                                             r")")
